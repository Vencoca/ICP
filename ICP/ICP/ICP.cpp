...
//void img_process_code(cv::VideoCapture & cap_device) {
...
int main()
{
    ...
    //Dobrý vìdìt že obèas nemáš kameru...
    //globals.capture = cv::VideoCapture(cv::CAP_DSHOW);
    // 
    //nastartuju vlákno a dám mu kameru
    std::thread img_thread(img_process_code);
    //std::thread img_thread = std::thread(img_process_code, globals.capture);
